...
try:
    from backend.risk.advanced_risk_manager import AdvancedRiskManager, RiskLimits
...
class PortfolioOptimizer:
    ...
    async def optimize_portfolio(self, 
                                ...
                                market_views: Optional[Dict[str, float]] = None) -> PortfolioAllocation:
        """
    ...
    async def _optimize_max_sharpe(self) -> PortfolioAllocation:
        ...
        try:
            ...
            return PortfolioAllocation(
                weights={symbol: float(weight) for symbol, weight in zip(self.asset_symbols, optimal_weights)},
    ...
    async def _optimize_black_litterman(self, market_views: Optional[Dict[str, float]] = None) -> PortfolioAllocation:
        ...
        try:
            ...
            if not market_views:
                ...
                return PortfolioAllocation(
                    weights={symbol: float(weight) for symbol, weight in zip(self.asset_symbols, market_weights)},
                    expected_return=float(portfolio_return),
                    expected_volatility=float(portfolio_vol),
                    sharpe_ratio=float(sharpe_ratio),
                    objective_value=float(sharpe_ratio),
    ...
    async def generate_rebalance_schedule(self, 
                                        ...
                                        start_date: Optional[datetime] = None) -> List[datetime]:
