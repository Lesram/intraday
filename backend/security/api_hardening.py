...
class InputSanitizer:
        ...
        if not allow_special:
            for pattern in cls.BLOCKED_PATTERNS:
                if pattern.search(value):
                    raise ValueError("String contains blocked pattern")
    ...
    @classmethod
    def validate_quantity(cls, qty: int | float | str) -> float:
    ...
    @classmethod
    def validate_price(cls, price: float | str | None) -> float | None:
