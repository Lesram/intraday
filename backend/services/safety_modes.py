...
trading_mode_operations = Counter(
    ...
    ['mode', 'operation', 'outcome']  # shadow/dry_run/live, submit_order/cancel_order, success/blocked/error
)
...
class TradingMode(Enum):
    ...
    def risk_level(self) -> int:
        ...
        return {
            TradingMode.SHADOW: 0,
            TradingMode.DRY_RUN: 1, 
            TradingMode.LIVE: 2
        }[self]
...
class SafetyModeManager:
    ...
    async def set_trading_mode(self, mode: TradingMode, authorized_by: str) -> bool:
        ...
        if new_risk > current_risk:
            logger.warning(f"Trading mode escalation: {self._current_mode} -> {mode} by {authorized_by}")
    ...
    def create_feature_flag(self, flag: FeatureFlag):
        ...
        active_safety_constraints.labels(
            constraint_type='feature_flag'
        ).set(len([f for f in self._feature_flags.values() if f.enabled]))
    ...
    def activate_kill_switch(self, 
                           ...
                           auto_reset_minutes: Optional[int] = None) -> bool:
        """
        ...
        if auto_reset_minutes:
            ...
            auto_reset_at = datetime.now(timezone.utc) + timedelta(minutes=auto_reset_minutes)
        ...
        active_safety_constraints.labels(
            constraint_type='kill_switch'
        ).set(len([ks for ks in self._kill_switches.values() if ks.active]))
    ...
    async def execute_order_with_safety(self,
                                      order_id: str,
                                      order_data: Dict[str, Any],
                                      user_id: str,
                                      execution_func: Callable) -> TradeExecutionResult:
        """
    ...
    async def _execute_shadow_mode(self,
                                 order_id: str,
                                 order_data: Dict[str, Any],
                                 execution_func: Callable,
                                 start_time: float) -> TradeExecutionResult:
    ...
    async def _execute_dry_run_mode(self,
                                  order_id: str,
                                  order_data: Dict[str, Any],
                                  execution_func: Callable,
                                  start_time: float) -> TradeExecutionResult:
        ...
        simulation_result = {
            'status': 'filled',
            'filled_quantity': order_data.get('quantity', 0),
            'average_price': order_data.get('price', 100.0) * random.uniform(0.999, 1.001),  # Small slippage
        ...
        trading_mode_operations.labels(
            mode=TradingMode.DRY_RUN.value,
            operation='submit_order',
            outcome='success'
        ).inc()
    ...
    async def _execute_live_mode(self,
                               order_id: str,
                               order_data: Dict[str, Any],
                               execution_func: Callable,
                               start_time: float) -> TradeExecutionResult:
...
async def submit_order_safely(order_id: str,
                            order_data: Dict[str, Any],
                            user_id: str,
                            execution_func: Callable) -> TradeExecutionResult:
