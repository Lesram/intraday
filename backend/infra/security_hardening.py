...
class SecuritySettings(BaseSettings):
    ...
    jwt_require_https: bool = Field(
        default=True,
        description="Require HTTPS for JWT tokens"
    )
    jwt_require_aud: bool = Field(
        default=True,
        description="Require audience claim in JWT"
    )
    jwt_require_iss: bool = Field(
        default=True,
        description="Require issuer claim in JWT"
    )
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not is_allowed:
            return JSONResponse(
                ...
                headers={
                    ...
                    "X-RateLimit-Remaining": str(max(0, rate_info["requests_allowed"] - rate_info["requests_made"])),
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, rate_info["requests_allowed"] - rate_info["requests_made"]))
