...
HISTOGRAM_BUCKETS: Final[dict[str, tuple[float, ...]]] = {
    ...
    "http_request_duration_seconds": (
        0.001, 0.005, 0.010, 0.025, 0.050,  # 1ms - 50ms (fast responses)
        0.100, 0.250, 0.500,                 # 100ms - 500ms (normal)
        1.0, 2.5, 5.0, 10.0,                # 1s - 10s (slow)
        float('inf')
    ),
    
    # Alpaca API latency: external service with network overhead
    "alpaca_http_latency_seconds": (
        0.050, 0.100, 0.250, 0.500,         # 50ms - 500ms (good)
        1.0, 2.0, 5.0, 10.0,                # 1s - 10s (acceptable)
        30.0, 60.0,                         # 30s - 60s (timeout range)
        float('inf')
    ),
    
    # Outbox dispatch latency: database + message queue operations
    "outbox_dispatch_latency_seconds": (
        0.001, 0.005, 0.010, 0.025,         # 1ms - 25ms (database ops)
        0.050, 0.100, 0.250,                # 50ms - 250ms (normal)
        0.500, 1.0, 2.5,                    # 500ms - 2.5s (batch ops)
        float('inf')
    ),
    
    # Database query duration: optimized for fast queries
    "db_query_duration_seconds": (
        0.001, 0.002, 0.005, 0.010,         # 1ms - 10ms (indexed queries)
        0.025, 0.050, 0.100,                # 25ms - 100ms (complex queries)
        0.250, 0.500, 1.0,                  # 250ms - 1s (analytical)
        float('inf')
    ),
    
    # Risk decision latency: critical path timing
    "risk_decision_latency_seconds": (
        0.001, 0.005, 0.010, 0.025,         # 1ms - 25ms (cache hits)
        0.050, 0.100, 0.250,                # 50ms - 250ms (computation)
        0.500, 1.0,                         # 500ms - 1s (slow path)
        float('inf')
    ),
    
    # Feature compute latency: ML pipeline timing
    "feature_compute_latency_seconds": (
        0.010, 0.025, 0.050, 0.100,         # 10ms - 100ms (simple features)
        0.250, 0.500, 1.0, 2.5,             # 250ms - 2.5s (complex features)
        5.0, 10.0,                          # 5s - 10s (heavy computation)
        float('inf')
    ),
}
