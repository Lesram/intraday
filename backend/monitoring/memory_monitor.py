        ...
        try:
            if self.data_file.exists():
                ...
                for snapshot_dict in data.get("snapshots", []):
                    ...
                    cutoff_time = datetime.now(UTC) - timedelta(hours=self.monitoring_duration_hours)
    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # Wait for next interval (blocking sleep is OK in dedicated thread)
                time.sleep(self.monitoring_interval_seconds)

            except Exception as e:
                logger.error(f"Error in memory monitoring loop: {e}")
                time.sleep(60)  # Wait a minute before retrying (in dedicated thread)
