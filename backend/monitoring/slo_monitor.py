...
# Daily caps tracking
# H-17 FIX: Remove account_id label to prevent cardinality explosion
# For multi-account setups, use account_bucket (e.g., "primary", "hedge") instead
...
class SLOMonitor:
    ...
    def __init__(self):
        self.stream_connect_time: datetime | None = None
        self.slo_targets = {
    ...
    def record_order_attempt(self, symbol: str, side: str, success: bool, error_type: str | None = None) -> None:
    ...
    def record_stream_reconnect(self, reason: str) -> None:
        ...
        self.stream_connect_time = datetime.now(UTC)
        stream_uptime_seconds.set(0)

        logger.warning(f"Stream reconnected due to: {reason}")

    def record_stream_message(self, message_type: str, broker_timestamp: datetime | None = None) -> None:
    ...
    def update_daily_caps(self, account_id: str, orders_count: int, notional_usd: float) -> None:
        ...
        # H-17 FIX: account_id logged but not used as label to prevent cardinality explosion
        logger.debug(f"Updating daily caps for account {account_id}: orders={orders_count}, notional=${notional_usd}")
        daily_orders_count.set(orders_count)
        daily_notional_usd.set(notional_usd)

    def update_stream_uptime(self) -> None:
        """Update stream uptime (call periodically)"""
        if self.stream_connect_time:
            uptime = (datetime.now(UTC) - self.stream_connect_time).total_seconds()
