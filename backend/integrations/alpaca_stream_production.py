    ...
    async def load_from_db(self) -> None:
        ...
        async for session in get_db_session():
            ...
            if last_event:
                ...
                self.last_event_ts = datetime.now(timezone.utc) - timedelta(hours=1)
...
class RobustAlpacaStream:
    ...
    def __init__(self, 
        ...
        self.guardrails: Optional[TransactionalGuardrails] = None
        
        # Connection tracking
        self.connect_time: Optional[datetime] = None
        self.reconnect_count = 0
        
    async def start(self, guardrails: Optional[TransactionalGuardrails] = None) -> None:
    ...
    async def _process_message(self, raw_message: str) -> None:
        ...
        try:
            ...
            # Update heartbeat
            self.stream_state.last_heartbeat = datetime.now(timezone.utc)
    ...
    async def _update_order_from_trade_event(self, session: Session, order: Any, trade_data: Dict[str, Any]) -> None:
        """Update order status based on trade event"""
        ...
        if event_type == "fill":
            ...
            order.filled_qty = (order.filled_qty or 0) + float(filled_qty)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.connect_time:
            uptime = (datetime.now(timezone.utc) - self.connect_time).total_seconds()
