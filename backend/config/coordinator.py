...
try:
    from backend.config.settings import AppSettings, get_settings as get_legacy_settings
...
try:
    from backend.config.base_settings import Settings as BaseSettings, get_settings as get_base_settings
    BASE_AVAILABLE = True
except ImportError:
...
class ConfigurationCoordinator:
    ...
    def get_alpaca_credentials(self) -> Dict[str, str]:
        ...
        # Environment variable fallback
        credentials['api_key'] = os.getenv('ALPACA_API_KEY', '')
        credentials['secret_key'] = os.getenv('ALPACA_SECRET_KEY', '')
