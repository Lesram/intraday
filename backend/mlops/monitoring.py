...
class StatisticalDriftDetector:
    ...
    def _simple_ks_test(reference_data: list[float], current_data: list[float]) -> tuple[float, float]:
        ...
        critical_value = 1.36 * ((n1 + n2) / (n1 * n2)) ** 0.5
    ...
    def _simple_psi(reference_data: list[float], current_data: list[float], bins: int) -> float:
        ...
        for i in range(len(ref_hist)):
            ref_prob = max(ref_hist[i] / len(reference_data), epsilon)
            cur_prob = max(cur_hist[i] / len(current_data), epsilon)
            psi += (cur_prob - ref_prob) * (cur_prob / ref_prob - 1) if ref_prob > 0 else 0
...
class ModelMonitor:
    ...
    def _extract_metric_value(self, metric_name: str, monitoring_results: dict[str, Any]) -> float | None:
        ...
        for result_type, result_data in monitoring_results.items():
