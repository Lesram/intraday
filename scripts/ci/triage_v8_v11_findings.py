...
1. **closed_in_unrecorded_wave**: the finding ID appears in either
   ...
   (b) a test file's source.  These are tracked-but-untagged closure
   candidates, not confirmed closures; ``--apply`` records them as
   deferred/reverification-required until behavioral close evidence is
   backfilled.
...
def _write_ledger_md(ledger: dict) -> None:
    """Keep the human-readable ledger mirror in sync after --apply."""
    lines: list[str] = []
    ...
    lines.append(
        ...
        "then reconciled by `scripts/ci/triage_v8_v11_findings.py --apply`.\n"
    )
    ...
    lines.append(
        "| ID | Version | Severity | Status | Wave Closed | "
        "Behavioral Test | Summary |"
    )
    lines.append("|---|---|---|---|---|---|---|")
    for r in sorted(
        ledger["findings"], key=lambda x: (x["status"] != "open", x["id"])
    ):
        lines.append(
...
def main() -> int:
    ...
    if args.apply:
        ...
        for f in findings:
            ...
            if t["v13_status"] == "closed_in_unrecorded_wave":
                ...
                f["deferral_reason"] = (
                    ...
                    "but the ledger lacks a behavioral close record. Treat "
                    "as deferred/reverification-required until a close commit "
                    "+ behavioral_test_path can be backfilled."
                )
        ...
        print("findings_ledger_v12.md updated from reconciled ledger")
