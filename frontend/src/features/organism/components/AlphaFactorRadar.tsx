...
const FACTORS = ['ml', 'breakout', 'institutional', 'momentum', 'momentum_quality', 'vol_price_div', 'regime'] as const;
const LABELS = ['ML', 'Breakout', 'Inst.', 'Mom.', 'Mom Q.', 'Vol-Price', 'Regime'];

const AlphaFactorRadar = ({ data, threshold }: { data: AlphaFactorScore | null; threshold?: number }) => {
  ...
  if (!data) {
    return (
      <Card title="Alpha Factors" size="small">
        <Text type="secondary">Select a symbol to see alpha factors</Text>
      </Card>
    );
  }

  return (
    <Card title={`Alpha Factors: ${data.symbol}`} size="small">
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <canvas ref={canvasRef} style={{ width: 220, height: 220 }} />
      </div>
      <div style={{ textAlign: 'center', marginTop: 4 }}>
        <Text type="secondary" style={{ fontSize: 11 }}>
          Composite: {data.composite_score.toFixed(3)} | Threshold: {data.threshold.toFixed(2)}
        </Text>
      </div>
    </Card>
  );
...
export default AlphaFactorRadar;
