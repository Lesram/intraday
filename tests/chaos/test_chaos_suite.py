...
class TestDatabaseChaos:
    ...
    async def test_database_transaction_rollback_on_failure(self, mock_db_session):
        ...
        retry_manager = resilience_manager.get_retry_manager("db_transaction", RetryConfig(max_attempts=1))
...
class TestExternalAPIResilienceChaos:
    ...
    async def test_market_data_feed_interruption(self, mock_alpaca_client):
        ...
        retry_manager = resilience_manager.get_retry_manager("websocket", RetryConfig(max_attempts=5))
...
class TestIdempotencyChaos:
    ...
    async def test_idempotent_order_submission_under_retries(self, mock_order_service):
        ...
        retry_manager = resilience_manager.get_retry_manager("order_service", RetryConfig(max_attempts=3))
    ...
    async def test_idempotency_with_partial_failures(self, mock_order_service):
        ...
        retry_manager = resilience_manager.get_retry_manager("partial_failure", RetryConfig(max_attempts=3))
        ...
        result = await retry_manager.execute_with_retry(
            ...
            idempotency_key="partial-order-456"
        )
