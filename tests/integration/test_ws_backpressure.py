...
class TestWebSocketBackpressure:
    ...
    async def ws_server_app(self):
        ...
        with patch("backend.websocket.broadcaster.WebSocketBroadcaster") as mock_broadcaster:
            ...
            async def mock_broadcast(message):
                ...
                mock_broadcaster_instance.messages_sent += len(mock_broadcaster_instance.active_connections)

                # Simulate dropping messages for slow consumers
                for conn_id, connection in mock_broadcaster_instance.active_connections.items():
                    if connection.get("slow", False) and len(connection.get("queue", [])) > 50:
    ...
    async def test_slow_consumer_message_dropping_policy(self, test_ws_server):
        ...
        assert final_queue[0]["sequence"] == 10  # First message is sequence 10 (0-9 were dropped)
    ...
    @pytest.mark.asyncio
    async def test_websocket_connection_recovery_after_backpressure(self, test_ws_server):
        ...
        mock_broadcaster.active_connections = {
            "recovering_client": client_state
        }
        ...
        assert mock_broadcaster.messages_dropped > 0  # Some messages were dropped during slow phase
    ...
    async def test_websocket_graceful_disconnection_under_load(self, test_ws_server):
        ...
        mock_broadcaster.active_connections = {
            "overloaded_client": {"queue": [], "slow": True, "disconnect_threshold": 100}
        }
    ...
    async def test_websocket_connection_limit_enforcement(self, test_ws_server):
        ...
        accepted_connections = [attempt for attempt in connection_attempts if attempt["accepted"]]
        rejected_connections = [attempt for attempt in connection_attempts if not attempt["accepted"]]
