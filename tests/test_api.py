...
class TestTradingSignalEndpoints:
    ...
    def test_get_trading_signal_success(
        self, client, sample_price_data, sample_features
    ):
        ...
        app_state["strategy_manager"].generate_combined_signal = AsyncMock(
            return_value=mock_signal
        )
        app_state["alpaca_client"].get_historical_data = AsyncMock(
            return_value=sample_price_data
        )
        app_state["feature_engineer"].compute_all_features = MagicMock(
    ...
    def test_get_trading_signal_missing_data(self, mock_state, client):
        ...
        mock_state.return_value["alpaca_client"].get_historical_data.return_value = (
            pd.DataFrame()
        )
...
class TestPortfolioEndpoints:
    ...
    @patch("backend.api.main.app_state", {"risk_manager": MagicMock()})
    @pytest.mark.unit
    def test_get_portfolio_status(self, client):
        ...
        app_state["risk_manager"].get_positions = MagicMock(
            return_value={"AAPL": {"quantity": 100, "market_value": 15000}}
        )
        # Return a dict instead of MagicMock for Pydantic validation
        app_state["risk_manager"].get_risk_metrics = MagicMock(
            return_value={"var_95": -0.02, "sharpe_ratio": 1.8}
        )
...
class TestTradingEndpoints:
    ...
    @patch(
        "backend.api.main.app_state",
        {"alpaca_client": AsyncMock(), "risk_manager": MagicMock()},
    )
    @pytest.mark.unit
    def test_submit_trade_success(self, client):
        ...
        app_state["risk_manager"].assess_position_risk = AsyncMock(
            return_value={"approved": True, "reason": "Risk check passed"}
        )

        # Mock successful order submission
        app_state["alpaca_client"].submit_order = AsyncMock(
            return_value={"id": "order_123", "status": "accepted"}
        )
    ...
    def test_submit_trade_risk_rejection(self, client):
        ...
        # Mock risk rejection
        app_state["risk_manager"].assess_position_risk = AsyncMock(
            return_value={"approved": False, "reason": "Position size too large"}
        )
...
class TestMarketDataEndpoints:
    ...
    @patch("backend.api.main.app_state", {"alpaca_client": AsyncMock()})
    @pytest.mark.unit
...
class TestSentimentEndpoints:
    ...
    @patch("backend.api.main.app_state", {"sentiment_analyzer": AsyncMock()})
    @pytest.mark.unit
    def test_get_sentiment(self, client):
        ...
        app_state["sentiment_analyzer"].get_aggregated_sentiment.return_value = (
            mock_sentiment
        )
...
class TestModelManagementEndpoints:
    ...
    def test_get_models_status(self, mock_state, client):
        ...
        mock_state.return_value = {"model_manager": AsyncMock()}
        ...
        mock_state.return_value["model_manager"].get_model_status.return_value = (
            mock_status
        )
...
class TestRiskEndpoints:
    ...
    def test_get_risk_metrics(self, mock_state, client):
        ...
        mock_state.return_value = {"risk_manager": AsyncMock()}
