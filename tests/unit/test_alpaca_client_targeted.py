...
Covers all major functionality with proper mocking and observability compatibility.
"""
...
class MockGetOrdersRequest:
    """Mock GetOrdersRequest."""
    def __init__(self, status=None, limit=50):
        self.status = status
        self.limit = limit


class MockBarsRequest:
    """Mock bars request."""
...
class MockLatestQuoteRequest:
    """Mock latest quote request."""
    def __init__(self, **kwargs):
        for key, value in kwargs.items():
            setattr(self, key, value)
...
def mock_alpaca_imports():
    ...
    with patch.dict('sys.modules', {
        ...
        with patch('backend.data.alpaca_client.TradingClient', MockTradingClient), \
             ...
             patch('backend.data.alpaca_client.GetOrdersRequest', MockGetOrdersRequest), \
             patch('backend.data.alpaca_client.StockBarsRequest', MockBarsRequest), \
             patch('backend.data.alpaca_client.CryptoBarsRequest', MockBarsRequest), \
             patch('backend.data.alpaca_client.StockLatestQuoteRequest', MockLatestQuoteRequest), \
...
@pytest.fixture
def alpaca_client_no_observability(mock_alpaca_imports):
    """Create AlpacaClient instance for testing with mocked observability."""
    with patch('backend.data.alpaca_client.validate_symbol', return_value=True), \
         ...
         patch('backend.data.alpaca_client.trace_span') as mock_trace, \
         patch('backend.data.alpaca_client.get_structured_logger') as mock_logger:
        
        # Mock the context manager for trace_span
        mock_span = Mock()
        mock_trace.return_value.__enter__ = Mock(return_value=mock_span)
        mock_trace.return_value.__exit__ = Mock(return_value=None)
        
        # Mock structured logger to avoid log_order_event issues
...
class TestAlpacaClientInitialization:
    ...
    def test_init_paper_trading(self, alpaca_client_no_observability):
        """Test initialization with paper trading."""
        client = alpaca_client_no_observability
...
class TestAlpacaClientHistoricalData:
    ...
    def test_get_historical_data_stock_success(self, alpaca_client_no_observability):
        """Test successful historical data retrieval for stocks."""
        result = alpaca_client_no_observability.get_historical_data(
            symbol="AAPL",
            timeframe="1Day",
            start="2024-01-01",
            end="2024-01-05"
    ...
    def test_get_historical_data_crypto_success(self, alpaca_client_no_observability):
    ...
    def test_get_historical_data_invalid_symbol(self, alpaca_client_no_observability):
        """Test historical data with invalid symbol."""
        with patch('backend.data.alpaca_client.validate_symbol', return_value=False):
            with pytest.raises(ValueError) as exc_info:
                alpaca_client_no_observability.get_historical_data("INVALID")
            
            assert "Invalid symbol format" in str(exc_info.value)
    
    def test_get_historical_data_invalid_timeframe(self, alpaca_client_no_observability):
        """Test historical data with invalid timeframe."""
        with pytest.raises(ValueError) as exc_info:
            alpaca_client_no_observability.get_historical_data("AAPL", timeframe="invalid")
        
        assert "Unsupported timeframe" in str(exc_info.value)
    
    def test_get_historical_data_default_dates(self, alpaca_client_no_observability):
        """Test historical data with default date range."""
        result = alpaca_client_no_observability.get_historical_data("AAPL")
        assert isinstance(result, pd.DataFrame)
    
    def test_get_historical_data_empty_result(self, alpaca_client_no_observability):
...
class TestAlpacaClientTrading:
    ...
    def test_submit_market_order_buy(self, alpaca_client_no_observability):
        """Test successful market buy order submission."""
        result = alpaca_client_no_observability.submit_order(
            symbol="AAPL",
            qty=100,
            side="buy",
            order_type="market"
    ...
    def test_submit_market_order_sell(self, alpaca_client_no_observability):
        ...
        assert isinstance(result, OrderResult)
        assert result.side == "buy"  # Mock returns "buy"
        assert result.quantity == 50
    
    def test_submit_limit_order_success(self, alpaca_client_no_observability):
    ...
    def test_submit_limit_order_no_price(self, alpaca_client_no_observability):
        """Test limit order submission without price."""
        with pytest.raises(ValueError) as exc_info:
            alpaca_client_no_observability.submit_order(
                symbol="AAPL",
                qty=100,
                side="buy",
                order_type="limit"
    ...
    def test_submit_order_invalid_symbol(self, alpaca_client_no_observability):
        """Test order submission with invalid symbol."""
        with patch('backend.data.alpaca_client.validate_symbol', return_value=False):
            with pytest.raises(ValueError) as exc_info:
                alpaca_client_no_observability.submit_order("INVALID", 100, "buy")
            
            assert "Invalid symbol format" in str(exc_info.value)
    
    def test_submit_order_invalid_type(self, alpaca_client_no_observability):
        """Test order submission with invalid order type."""
        with pytest.raises(ValueError) as exc_info:
            alpaca_client_no_observability.submit_order("AAPL", 100, "buy", order_type="invalid")
        
        assert "Unsupported order type" in str(exc_info.value)
    
    def test_cancel_order_success(self, alpaca_client_no_observability):
        """Test successful order cancellation."""
        result = alpaca_client_no_observability.cancel_order("test_order_123")
        assert result is True
    
    def test_cancel_order_failure(self, alpaca_client_no_observability):
        """Test order cancellation failure."""
        with patch.object(MockTradingClient, 'cancel_order_by_id', side_effect=Exception("Not found")):
            result = alpaca_client_no_observability.cancel_order("invalid_order")
...
class TestAlpacaClientAccountManagement:
    ...
    def test_get_account_status_success(self, alpaca_client_no_observability):
        """Test successful account status retrieval."""
        result = alpaca_client_no_observability.get_account_status()
    ...
    def test_get_recent_orders_success(self, alpaca_client_no_observability):
        ...
        result = alpaca_client_no_observability.get_recent_orders(limit=10)
...
class TestAlpacaClientPricing:
    ...
    def test_get_current_price_stock_success(self, alpaca_client_no_observability):
        """Test successful current price retrieval for stocks."""
        price = alpaca_client_no_observability.get_current_price("AAPL")
        assert price == 152.0  # Average of bid/ask
    
    def test_get_current_price_crypto_success(self, alpaca_client_no_observability):
        """Test successful current price retrieval for crypto."""
        price = alpaca_client_no_observability.get_current_price("BTC/USD")
        assert price == 45750.0
    
    def test_get_current_price_not_found(self, alpaca_client_no_observability):
        ...
        with patch.object(MockStockHistoricalDataClient, 'get_stock_latest_quote', return_value={}):
            price = alpaca_client_no_observability.get_current_price("NOTFOUND")
...
class TestAlpacaClientStreaming:
    ...
    @pytest.mark.asyncio
    async def test_connect_data_stream_stocks(self, alpaca_client_no_observability):
        ...
        try:
            await asyncio.wait_for(
                alpaca_client_no_observability.connect_data_stream(
                    symbols=["AAPL", "GOOGL"],
                    on_bar=callback
                ),
                timeout=1.0
        ...
        assert alpaca_client_no_observability.connected is True
    
    @pytest.mark.asyncio
    async def test_connect_data_stream_crypto(self, alpaca_client_no_observability):
    ...
    def test_add_data_callback(self, alpaca_client_no_observability):
        """Test adding data callback."""
        callback = Mock()
        alpaca_client_no_observability.add_data_callback(callback)
        assert callback in alpaca_client_no_observability.data_callbacks
    
    def test_disconnect(self, alpaca_client_no_observability):
        """Test disconnection from streams."""
        alpaca_client_no_observability.connected = True
        alpaca_client_no_observability.disconnect()
        assert alpaca_client_no_observability.connected is False


class TestAlpacaClientUtilities:
    """Test utility functionality."""
    
    def test_rate_limiting(self, alpaca_client_no_observability):
        """Test rate limiting mechanism."""
        start_time = time.time()
        
        # Make multiple rapid requests
        alpaca_client_no_observability._rate_limit()
        alpaca_client_no_observability._rate_limit()
        alpaca_client_no_observability._rate_limit()
    ...
    def test_destructor_cleanup(self, alpaca_client_no_observability):
        """Test cleanup on destruction."""
        alpaca_client_no_observability.connected = True
        alpaca_client_no_observability.__del__()
        assert alpaca_client_no_observability.connected is False
