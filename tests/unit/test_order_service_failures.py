...
from backend.infra.repositories.orders import OrdersRepo, OrderNotFoundError, DuplicateOrderError
...
class TestOrderServiceFailureHandling:
    ...
    async def test_outbox_repo_failure_after_order_creation(self):
        ...
        # Outbox fails
        mock_outbox_repo.add_order_submit_event.side_effect = Exception("Outbox service unavailable")
        ...
        with pytest.raises(Exception, match="Outbox service unavailable"):
            await service.submit_symbol_order(
                ...
                side="buy", 
                qty=100.0,
                idempotency_key="outbox-fail-key"
            )
...
class TestOrderServiceStrategyIntegration:
    ...
    async def test_plan_and_submit_single_signal(self):
        ...
        results = await service.plan_and_submit(
            signals=[signal],
            idempotency_key="strategy-key-1"
        )
    ...
    async def test_plan_and_submit_risk_blocked_orders(self):
        ...
        results = await service.plan_and_submit(
            signals=[signal],
            idempotency_key="risk-blocked-key"
        )
    ...
    async def test_plan_and_submit_mixed_results(self):
        ...
        signals = [
            TradingSignal(symbol="AAPL", signal_strength=0.8, confidence=0.9, timestamp=datetime.utcnow(), features={}),
            TradingSignal(symbol="RISKY_STOCK", signal_strength=0.7, confidence=0.8, timestamp=datetime.utcnow(), features={}), 
            TradingSignal(symbol="STABLE_STOCK", signal_strength=0.1, confidence=0.5, timestamp=datetime.utcnow(), features={})
        ]
