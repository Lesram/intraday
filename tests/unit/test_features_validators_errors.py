...
from backend.features.types import LookaheadLeakError, SchemaValidationError
...
class TestOHLCVValidationErrors:
    ...
    def test_validate_ohlcv_non_monotonic_timestamps(self):
        ...
        dates = [
            ...
            datetime(2023, 1, 3, tzinfo=timezone.utc),  # Skip day 2
            datetime(2023, 1, 2, tzinfo=timezone.utc),  # Go back to day 2 (non-monotonic)
        ]
...
class TestLookaheadDetectionEdgeCases:
    ...
    def test_guard_no_lookahead_clean_features(self):
        ...
        features = pd.DataFrame({
            ...
            "rsi": price.rolling(14).apply(lambda x: 50 + np.random.randn())  # Random oscillator
        }, index=dates)
    ...
    def test_guard_no_lookahead_mismatched_indices(self):
        ...
        dates_price = pd.date_range('2023-01-02', periods=100, freq='1min', tz='UTC')  # Different dates
    ...
    def test_guard_no_lookahead_synthetic_specific_columns(self):
        ...
        with pytest.raises(LookaheadLeakError):
            guard_no_lookahead_synthetic(features, feature_cols=["suspicious_feature"], accuracy_threshold=0.8)

        # Test only good features - should pass
        guard_no_lookahead_synthetic(features, feature_cols=["good_feature", "another_good_feature"])
...
class TestValidatorErrorHandling:
    ...
    def test_lookahead_detection_performance_with_large_datasets(self):
        ...
        features = pd.DataFrame({
            f"feature_{i}": np.random.randn(n_periods) for i in range(10)
        }, index=dates)
    ...
    def test_synthetic_lookahead_with_edge_case_correlations(self):
        ...
        with pytest.raises(LookaheadLeakError):
            guard_no_lookahead_synthetic(features, feature_cols=["perfect_positive"], accuracy_threshold=0.9)

        with pytest.raises(LookaheadLeakError):
            guard_no_lookahead_synthetic(features, feature_cols=["perfect_negative"], accuracy_threshold=0.9)

        # Step function might also be detected depending on threshold
        try:
            guard_no_lookahead_synthetic(features, feature_cols=["step_function"], accuracy_threshold=0.9)
...
class TestRealWorldValidationScenarios:
    ...
    def test_market_hours_gap_handling(self):
        ...
        trading_dates = pd.date_range('2023-01-01', periods=10, freq='B', tz='UTC')  # Business days only
