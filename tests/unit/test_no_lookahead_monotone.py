...
class TestMonotoneLookaheadDetection:
    ...
    def test_monotone_series_clean_features(self):
        ...
        features = pd.DataFrame({
            ...
            "random_rsi": pd.Series(np.random.uniform(20, 80, n_periods), index=dates),
            "random_macd": pd.Series(np.random.normal(0, 1, n_periods), index=dates),
        }, index=dates)

        # Should pass without raising exception
        guard_no_lookahead_synthetic(features, list(features.columns), accuracy_threshold=0.8)
    ...
    def test_monotone_series_detects_lookahead(self):
        ...
        features = pd.DataFrame({
            # Good feature (random)
            "good_feature": pd.Series(np.random.normal(0, 1, n_periods), index=dates),
            ...
            "subtle_leak": pd.Series(np.random.randn(n_periods), index=dates).shift(-5),
        }, index=dates)

        # Should detect lookahead bias and raise exception
        with pytest.raises(LookaheadLeakError, match="suspicious correlation with synthetic monotone"):
            guard_no_lookahead_synthetic(features, list(features.columns), accuracy_threshold=0.6)
    ...
    def test_monotone_detection_specific_columns(self):
        ...
        features = pd.DataFrame({
            # Clean feature
            "clean_feature": pd.Series(np.random.normal(0, 1, n_periods), index=dates),

            # Leaky feature (correlated with index)
            "leaky_feature": pd.Series(np.arange(n_periods) * 0.1, index=dates),

            # Another clean feature
            "another_clean": pd.Series(np.random.uniform(-1, 1, n_periods), index=dates),
        }, index=dates)

        # Test only the clean feature - should pass
        guard_no_lookahead_synthetic(features, ["clean_feature", "another_clean"], accuracy_threshold=0.7)

        # Test including the leaky feature - should fail
        with pytest.raises(LookaheadLeakError):
            guard_no_lookahead_synthetic(features, ["clean_feature", "leaky_feature"], accuracy_threshold=0.7)
    ...
    def test_corrected_features_pass_test(self):
        ...
        base_monotone_like = pd.Series(np.arange(n_periods) + np.random.normal(0, 0.1, n_periods), index=dates)
        ...
        corrected_features = pd.DataFrame({
            ...
            "differenced": base_monotone_like.diff().shift(5),  # Differenced and lagged
        }, index=dates)
...
class TestRealWorldLookaheadScenarios:
    ...
    def test_technical_indicators_properly_lagged(self):
        ...
        features = pd.DataFrame({
            ...
            "roc_5": (price / price.shift(5) - 1).shift(1),  # Rate of change, lagged
            ...
            "price_position": ((price - price.rolling(50).min()) /
                              (price.rolling(50).max() - price.rolling(50).min())).shift(1),
        }, index=dates)
    ...
    def test_volume_indicators_no_lookahead(self):
        ...
        features = pd.DataFrame({
            ...
            "vwap_deviation": (price - (price * volume).rolling(20).sum() / volume.rolling(20).sum()).shift(1),
            "volume_price_trend": ((price * volume).rolling(10).sum()).shift(1),

            # On-balance volume (properly calculated)
            "obv": ((price.diff() > 0).astype(int) * volume -
                   (price.diff() < 0).astype(int) * volume).cumsum().shift(1),
        }, index=dates)
    ...
    def test_regime_indicators_no_lookahead(self):
        ...
        regime_changes = np.random.choice([0, 1], n_periods, p=[0.95, 0.05])  # 5% chance of regime change
        volatility_regime = np.cumsum(regime_changes * np.random.choice([-1, 1], n_periods))
        volatility_regime = np.clip(volatility_regime, 0.5, 3.0)  # Keep vol between 0.5% and 3%

        price = pd.Series(100 * np.exp(np.cumsum(np.random.randn(n_periods) * volatility_regime / 100)),
                         index=dates)
        ...
        features = pd.DataFrame({
            ...
            "vol_ratio": (returns.rolling(20).std() / returns.rolling(60).std()).shift(1),

            # Trend strength indicators
            "trend_strength": np.abs(returns.rolling(20).mean() / returns.rolling(20).std()).shift(1),

            # Regime persistence
            "vol_persistence": returns.rolling(20).std().rolling(5).std().shift(1),

            # Drawdown indicators
            "drawdown": ((price - price.rolling(50).max()) / price.rolling(50).max()).shift(1),
        }, index=dates)
...
class TestEdgeCases:
    ...
    def test_insufficient_data_handling(self):
        ...
        guard_no_lookahead_synthetic(features, accuracy_threshold=0.8)  # Should not raise
