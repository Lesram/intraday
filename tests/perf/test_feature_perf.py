...
class TestFeatureComputePerformance:
    ...
    async def test_feature_computation_performance(self, performance_ohlcv_data):
        ...
        if n_periods <= 1000:
            assert compute_time < 2.0, f"Feature computation took {compute_time:.3f}s for {n_periods} periods"
        elif n_periods <= 5000:
            assert compute_time < 8.0, f"Feature computation took {compute_time:.3f}s for {n_periods} periods"
        else:  # 10000 periods
            assert compute_time < 20.0, f"Feature computation took {compute_time:.3f}s for {n_periods} periods"

        # Throughput metrics
        throughput = n_periods / compute_time
        print(f"Feature computation throughput: {throughput:.0f} periods/second for {n_periods} periods")
        ...
        assert memory_mb < n_periods * 0.01, f"Feature memory usage {memory_mb:.2f}MB too high for {n_periods} periods"
    ...
    async def test_feature_frame_build_performance(self, performance_ohlcv_data):
        ...
        assert build_time < 1.0, f"FeatureFrame build took {build_time:.3f}s for {n_periods} periods"
...
class TestValidationPerformance:
    ...
    def large_feature_dataset(self):
        ...
        for i in range(n_features):
            if i % 4 == 0:  # Price-based features
                ...
                features_data[f'price_feature_{i}'] = base_price * (1 + np.random.normal(0, 0.01, n_periods))
            elif i % 4 == 1:  # Oscillator features
                features_data[f'oscillator_{i}'] = np.sin(np.arange(n_periods) / 50) + np.random.normal(0, 0.1, n_periods)
        ...
        target = pd.Series(np.random.normal(0, 0.01, n_periods), index=dates, name='target')
    ...
    def test_ohlcv_validation_performance(self):
        ...
        assert validation_time < 1.0, f"OHLCV validation took {validation_time:.3f}s for {n_periods} periods"
    ...
    def test_lookahead_detection_performance(self, large_feature_dataset):
        ...
        assert correlation_time < 5.0, f"Correlation lookahead detection took {correlation_time:.3f}s"
        assert synthetic_time < 10.0, f"Synthetic lookahead detection took {synthetic_time:.3f}s"

        print(f"Lookahead detection performance for {n_features} features, {n_periods} periods:")
...
class TestAlignmentPerformance:
    ...
    def test_single_timeframe_alignment_performance(self, alignment_test_data):
        ...
        assert alignment_time < 2.0, f"Single TF alignment took {alignment_time:.3f}s for {n_periods} periods"
        ...
        print(f"Single timeframe alignment: {alignment_time:.3f}s for {n_periods} periods")

    def test_multitimeframe_alignment_performance(self):
        """Test multi-timeframe alignment performance."""
        # Create multi-timeframe data
        n_periods_1min = 5000
        dates_1min = pd.date_range('2023-01-01', periods=n_periods_1min, freq='1min', tz='UTC')
...
class TestMemoryUsageValidation:
    ...
    def test_feature_pipeline_memory_usage(self):
        ...
        assert memory_increase < expected_data_size * 5, \
    ...
    def test_feature_frame_memory_efficiency(self):
        ...
        frame_memory = feature_frame.data.memory_usage(deep=True).sum() / 1024 / 1024  # MB

        # FeatureFrame should not significantly increase memory usage
        memory_ratio = frame_memory / original_memory
        assert memory_ratio < 1.2, f"FeatureFrame uses {memory_ratio:.2f}x original memory"
...
class TestConcurrencyPerformance:
    ...
    async def test_concurrent_feature_computation(self):
        ...
        for i in range(n_concurrent):
            dates = pd.date_range(f'2023-0{i+1}-01', periods=n_periods, freq='1min', tz='UTC')
            np.random.seed(42 + i)
