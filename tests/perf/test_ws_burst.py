...
class TestResults:
    ...
    def success_rate(self) -> float:
        ...
        return (self.successful_connections / self.total_connections * 100) if self.total_connections > 0 else 0.0
    ...
    def avg_connection_duration(self) -> float:
        ...
        durations = [m.connection_duration for m in self.connection_metrics if m.connection_duration > 0]
...
class WebSocketBurstTester:
    ...
    async def create_connection(self, connection_id: str, endpoint: str = "/ws/market-data") -> ConnectionMetrics:
    ...
    async def burst_test(self, 
                        ...
                        connection_ramp_rate: float = 10.0) -> TestResults:
        """
        ...
        """
        logger.info(f"Starting WebSocket burst test: {num_connections} connections, {duration_seconds}s duration")
    ...
    async def connection_churn_test(self, 
                                   connections_per_wave: int = 20,
                                   num_waves: int = 5,
                                   wave_interval: float = 10.0) -> TestResults:
        """
        ...
        """
        logger.info(f"Starting connection churn test: {num_waves} waves of {connections_per_wave} connections")
    ...
    def _log_results(self):
        ...
        if self.results.connection_metrics:
            ...
            if all_latencies:
                ...
                logger.info(f"P95 latency: {statistics.quantiles(all_latencies, n=20)[18]*1000:.1f}ms")
...
class TestWebSocketPerformance:
    ...
    async def test_websocket_burst_small(self, tester):
        ...
        assert results.messages_per_second >= 1.0, f"Message throughput too low: {results.messages_per_second}"
    ...
    async def test_websocket_burst_medium(self, tester):
        ...
        assert results.messages_per_second >= 5.0, f"Message throughput too low: {results.messages_per_second}"
    ...
    async def test_websocket_connection_churn(self, tester):
        ...
        results = await tester.connection_churn_test(
            connections_per_wave=15,
            num_waves=3,
            wave_interval=8.0
        )
        
        # Churn-specific thresholds
        assert results.success_rate >= 80.0, f"Success rate too low under churn: {results.success_rate}%"
