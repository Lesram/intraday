...
| Scope | Files | Lines |
|-------|-------|-------|
| backend/ (total) | 297 | 133,067 |
| backend/organism/ | 35 | 20,603 |
| backend/ (non-organism) | 262 | 112,464 |
| tests/ | 390 | 119,877 |
...
### Organism (35 modules, 20,603 lines)

The living trading organism — the autonomous trading brain.

| Category | Modules | Key Files |
|----------|---------|-----------|
| Core engine | 5 | `live_engine.py` (4,076), `scheduler.py`, `runner.py`, `routes.py`, `governance.py` |
...
| Risk & sizing | 4 | `kelly_sizer.py`, `adaptive_exits.py`, `pyramider.py`, `sector_map.py` |
...
| Infrastructure | 5 | `replay_simulator.py`, `promotion.py`, `nightly_scheduler.py`, `feature_store.py`, `universe_selector.py` |

### Non-Organism Backend (262 files, 112,464 lines)
...
| Other | 71 | ~25,000 | models, migrations, analytics, features, utils, research, optimization, security, observability |
...
| Other root tests | ~130 | ~1,000+ | Production, diagnostics, security, utilities |
...
| `backend_file_inventory.json` | Every backend .py file with lines, role, classes |
| `test_file_inventory.json` | Every test file with test counts and coverage scope |
| `docs_file_inventory.json` | Architecture docs inventory |
| `config_file_inventory.json` | Docker, env, CI workflow, Python build config |
| `strategy_surface_manifest.json` | 10-stage trading pipeline with every participating module |
| `full_backend_tree.txt` | Raw file listing of all backend .py files |
| `full_tests_tree.txt` | Raw file listing of all test .py files |
