...
SHA: `32a3474` (merge commit of PR #3 into main)
...
| Property | Value |
|----------|-------|
| SHA | `32a3474` (PR #3 merge into main) |
| Branch | `main` |
| Mode | Paper trading, development environment |
| Timeframe | 1Min (container env) |
| Learning mode | Active (142 trades, threshold 200) |
| Universe | 30 symbols (.env), code default 22 |
| Brain state | gen 0, 35 runs, 142 trades, PnL -$944.74 |
| ML status | Not trained, learning mode weights active |
| Evolution | Frozen (142 < 300 trade threshold) |
| Docker | 3 containers (api, postgres, redis), paper compose |
...
Scores every symbol in the universe by combining ML prediction, breakout strength, volume profile, and regime alignment into a single `composite_score`. Returns the top-N candidates (N=5) sorted by score. Includes a stocks-in-play overlay that boosts symbols with high relative volume and gap. Contains inverse ETF definitions (DOG, PSQ, RWM, SH).
...
**Current state**: Learning mode active (142 trades). Fitness gate OFF, ML weight 0%, entry throttle 12/hr.

### `sector_map.py` (96 lines)
Static mapping of the universe to GICS sectors. Enforces max 2 positions per sector for diversification.
...
Two sizing modes:
- **Learning mode** (< 200 trades): Fixed ATR-dollar risk. `size = equity * 0.001 / stop_distance`. Kelly is OFF. Dollar-risk cap 0.10% equity + notional cap 5% per position.
...
**Current state**: Learning mode. Kelly OFF. Risk budget 0.1% equity per trade.
...
- **ATR trailing stops** with regime-specific multipliers (trending_up=3.5x, trending_down=2.5x, chop=2.5x, high_vol=4.0x, low_vol=3.0x, stress=2.5x, unknown=3.0x)
...
**Evolution is frozen for the first 300 trades.** Current state: 142 trades, evolution frozen.
...
Preserves knowledge across brain resets using distillation: extracts regime-conditioned trade statistics and feature importance from the old brain, then warm-starts the new brain. Warm-start gated by 300-trade freeze.
...
3. **Evolution freeze (300 trades)** prevents parameter adaptation from corrupting the learning signal during early operation. Currently active (142 trades).

4. **Three layers of risk control**: organism gates (live_engine), governance (drawdown kill at 20%), and institutional risk manager (backend/risk). They operate independently.
...
8. **Exploration is fully removed** — no execution path exists for exploration trades. This was done intentionally (improve9/hardening H1).

9. **Universe size mismatch**: .env specifies 30 symbols but code default is 22. The .env value takes precedence at runtime.

10. **Max positions is 15** (from .env), higher than the code default of 8. Alpha top_n remains 5.

11. **Brain is early-stage**: gen 0, 35 runs, 142 trades, cumulative PnL -$944.74. ML not yet trained.
